$(document).ready(function () {
    ...
    var searchEngineId = $("#search_engine_id").combobox('getValue');
    $.ajax({
        url: "../common/getSitesByDictionaryId.do", // 获取本业务和竞争对手
        type: "post",
        data: {"dictionaryId": dictionaryId, "searchEngineId": searchEngineId, "needAll": 0},
        cache: false,
        async: false,
        success: function (data, status) {
            if (data == null) return;
            $("#site_id").combobox('loadData', data);
        },
        error: function () {
            return false;
        },
